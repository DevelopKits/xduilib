...
{
	int iSelIdx = -1;
	if (m_bMultiSel) iSelIdx = m_selectedItems.Find(pControl);
	...
	if (!m_bMultiSel) {
		...
		m_selectedItems.Remove(iSelIdx);
	}
    return true;
}
...
{
    return m_bMultiSel ? -1 : m_iCurSel;
}

bool CListUI::IsMultiSel() const {
	return m_bMultiSel;
}

void CListUI::SetMultiSel(bool bMultiSel /*= true*/) {
	m_bMultiSel = bMultiSel;
}

CStdPtrArray& CListUI::GetSelItems() {
	return m_selectedItems;
}

bool CListUI::UnSelectItem(int iIndex, bool bTakeFocus) {
	...
	if (!m_bMultiSel) {
		...
		EnsureVisible(iOldSel);
		if( bTakeFocus ) pControl->SetFocus();
		if( m_pManager != NULL ) {
			m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMUNSELECT, iOldSel, -1);
		}

		return true;
	} else {
		int idx = m_selectedItems.Find(pListItem);
		if (idx != -1) return false;

		pListItem->Select(false);
		m_selectedItems.Remove(idx);
		EnsureVisible(iIndex);
		if( bTakeFocus ) pControl->SetFocus();
		if( m_pManager != NULL ) {
			m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMUNSELECT, iIndex, -1);
		}
...
bool CListUI::IsItemSelected(int iIndex) {
	if (!m_bMultiSel) return m_iCurSel == iIndex;
	for (int i=0; i < m_selectedItems.GetSize(); i++) {
		CControlUI* pControl = (CControlUI*)m_selectedItems.GetAt(i);
		IListItemUI* pListItem = static_cast<IListItemUI*>(pControl->GetInterface(_T("ListItem")));
		if (pListItem->GetIndex() == iIndex) return true;
	}
	return false;
}
...
bool CListUI::SelectItem(int iIndex, bool bTakeFocus)
{
	...
	if (!m_bMultiSel) {
		...
		m_selectedItems.Add(pControl);
		EnsureVisible(iIndex);
		if( bTakeFocus ) pControl->SetFocus();
		if( m_pManager != NULL ) {
			m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMSELECT, iIndex, -1);
		}
